(function(){
if(window.VarsaflowWidgetLoaded)return;
window.VarsaflowWidgetLoaded=true;
function load(){
 const d=document.createElement('div');
 d.innerHTML=`<!-- ============================================================
  Varsaflow.ai — Shopify Chat Widget
  Drop this snippet (or your Shopify theme.liquid) right before </body>.
  Change the webhook URL in the script section marked WEBHOOK_URL.
============================================================= -->
<div id="varsaflow-chat-widget">
  <button id="varsaflow-chat-bubble" class="vf-bubble-btn" aria-label="Open chat">
    <img src="./image.png" alt="" class="vf-bubble-icon">
  </button>
  ...
  <div id="varsaflow-chat-window" class="vf-window hidden" role="dialog" aria-label="Varsaflow chat">
    ...
    <div class="vf-header">
      <div class="vf-header-left">
        <img src="./image.png" alt="" class="vf-header-mark">
    ...
    <div id="varsaflow-chat-messages" class="vf-messages">
      <div class="vf-message vf-bot">
        <img src="./image.png" alt="" class="vf-avatar">
...
<style>
#varsaflow-chat-widget, #varsaflow-chat-widget * {
...
.vf-bubble-btn {
  ...
  box-shadow: 0 8px 24px rgba(13, 21, 38, 0.35);
  transition: transform 0.25s cubic-bezier(0.34, 1.56, 0.64, 1), box-shadow 0.25s ease;
}
.vf-bubble-btn:hover {
  transform: scale(1.06);
  box-shadow: 0 10px 28px rgba(13, 21, 38, 0.45);
}
.vf-bubble-btn:active {
...
.vf-window {
  ...
  background: #ffffff;
  border-radius: 18px;
  box-shadow: 0 16px 48px rgba(13, 21, 38, 0.22), 0 4px 14px rgba(13, 21, 38, 0.1);
  display: flex;
...
/* ---------- Header ---------- */
.vf-header {
  background: linear-gradient(135deg, var(--vf-navy-900) 0%, var(--vf-ink) 60%, #0f3460 100%);
  padding: 14px 16px;
  display: flex;
  align-items: center;
...
.vf-user .vf-bubble {
  background: linear-gradient(135deg, var(--vf-blue) 0%, var(--vf-teal) 100%);
  color: #ffffff;
...
.vf-typing-bubble span {
  width: 6px;
  height: 6px;
  border-radius: 50%;
  background: #b7c0d4;
  animation: vf-bounce 1.2s infinite ease-in-out;
}
...
.vf-send-btn {
  ...
  box-shadow: 0 3px 10px rgba(61, 110, 224, 0.3);
}
...
(function () {
  // ---- CONFIG ----
  var WEBHOOK_URL = "https://harshjot717.app.n8n.cloud/webhook-test/caf4f8c2-8b16-4d1b-869a-0b75982b372e"; // demo webhook — replace with your real endpoint
  ...
  function addMessage(text, sender) {
    ...
    if (sender === 'bot') {
      row.innerHTML = '<img src="./image.png" alt="" class="vf-avatar">' + bubbleHtml;
    } else {
